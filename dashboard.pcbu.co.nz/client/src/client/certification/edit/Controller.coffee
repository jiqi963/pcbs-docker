...
module.exports = (
  ...
  certificationId = $routeParams.certificationId
  ...
  populateDistinctNames = (classification) ->
    CertificationFactory.one('distinct')
      .customGETLIST 'name', {classification: classification, business: businessId}
      .then (items) ->
  ...
  $scope.edit = () ->
    ClientNavFactory.certificationEdit $scope.business._id, $scope.certification._id
  ...
  $scope.navBack = () ->
    group = if $scope.certification.isNew then groupName else $scope.certification.name

    if group? and classification?
      return ClientNavFactory.certificationGroup businessId, classification, group
